...
class BackupStorage():
    ...
    def init_parser(self):
        ...
        parser.add_argument('--dbhost', action='store',
                            help='host of main database')
        parser.add_argument('--dbport', action='store',
                            help='port of main database')
    ...
    def check_remote_backup_path(self, args, backup_dir):
        ...
        try:
            ...
            create_dir = self.execute_on_remote_shell(self.ssh_obj ,cmd)
            if create_dir and not create_dir.get('status'):
                print("Error while creating directory on the remote server - ", create_dir.get('message'))
                raise Exception(f"Error while creating directory on the remote server - {create_dir.get('message')}")
            cmd = "chmod -R 777 %s"%(backup_dir)
            upd_permission = self.execute_on_remote_shell(self.ssh_obj,cmd)
            if upd_permission and not upd_permission.get('status'):
                print(f"Error while updating permissions - {upd_permission.get('message')}")
    ...
    def create_client_url(self, url, dbname):
        """
        ...
        client_url += 'saas/database/backup?db=%s'%dbname
        return client_url
    ...
    def manage_backup_files(self, args):
        ...
        response = dict(status=False)
        self.client_url = self.create_client_url(args.url, args.dbname)
        kwargs = {
            'db_host': args.dbhost,
            'db_port': args.dbport,
        }
        try:
            ...
            self.database_entry(args.maindb, args.dbuser, args.dbpassword, args.dbname, self.filename, args.processid, backup_dir+'/', self.backup_file_path, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), status="Success", message=msg, kwargs=kwargs)
            response.update(status=True, message=msg)
        except Exception as e:
            msg = 'Failed at ' + str(self.backup_time or datetime.datetime.now()) + ' ' + str(e)
            self.database_entry(args.maindb, args.dbuser, args.dbpassword, args.dbname, self.filename, args.processid, backup_dir+'/', self.backup_file_path if self.backup_file_path else self.remote_backup_file_path if self.remote_backup_file_path else '', datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), status="Failure", message=msg, kwargs=kwargs)
    ...
    def _create_local_backup(self, args, vals):
        ...
        if args.is_remote_client and eval(args.is_remote_client):
            temp_backup_dir = args.temp_bkp_path or '/tmp'

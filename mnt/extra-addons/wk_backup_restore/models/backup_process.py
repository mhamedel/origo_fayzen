...
class BackupProcess(models.Model):
    ...
    def run_backup_now(self):
        """
            Method called from Button 'Backup Now', 
            Method called the script to update already created cron to run in next 5 minutes.
        """
        odoo_user_timezone = self.env.user.tz or 'UTC' #If Set
        res = manage_backup_crons.immediate_run_cron(db_name=self.db_name, process_id=str(self.id), frequency=self.frequency, frequency_cycle=self.frequency_cycle, timezone=odoo_user_timezone)
        if res.get('success'):
            ...
            return {
                ...
                'params': {
                    'message': _("Backup in progress. Please check back in 10 minutes."),
                    'type': 'success',
                    'sticky': True,
    ...
    def unlink(self):
        for obj in self:
            if obj.state not in ['draft','cancel','confirm']:
                raise UserError("Not allowed")
            return super(BackupProcess, obj).unlink()
    ...
    def confirm_process(self):
        ...
        try:
            ...
            raise UserError(f"The path for Backup cron log file doesn't exists: {str(manage_backup_crons.LOG_FILE_PATH)}. Kindly create it.")
        except Exception as e:
    ...
    def remove_old_backups(self):
        ...
        processes = self.env['backup.process'].sudo().search([('state', 'in', ['running', 'cancel']),('enable_retention', '=', True)])
